...
function nextStepGen(nextStep, responseHandler) {
    return function doNext(err, data) {
        ...
        console.log(err);
        console.log(data);
        return err ? nextStep(1) : nextStep();
    };
}
...
function printResponse(functionName, response) {
    process.stdout.write(`Info: ${functionName}, ${response.code}, ` +
        `${response.message}, ${JSON.stringify(response.body)}${eol}`);
}
/**
 ...
 *  @returns {void}
 */
function createGen(command, enteredArgs, nextStep) {
    const maxArgNumber = getMaxArgNumber(command);
    let data = {};

    if (!checkArgs(command, enteredArgs, maxArgNumber)) {
        data = getFormattedDataForRequest(expectedArgs[command],
            enteredArgs);
        if (command === 'create-access-key' && data.userName === '0') {
            delete data.userName;
        }
        client.create(command, data, nextStepGen(nextStep,
 ...
 *  @param {Function} nextStep - Either prompts or exits depending on the mode
 *  @returns {Number} - Exit code is 3 when command doesn't exist
 *  More exit codes will be implemented as we implement more functions
 */
...
function runCommand(enteredArgs, nextStep) {
    ...
    case 'create-account' :
        return createGen('create-account', enteredArgs, nextStep);
/*    case 'generate-credential-report' :
        generateCredentialReport(enteredArgs);
        break;
    case 'delete-account' :
        deleteAccount(enteredArgs);
        break;
    case 'create-group' :
    ...
    case 'delete-group-policy' :
        ...
        break;*/
    case 'create-user':
        return createGen('create-user', enteredArgs, nextStep);
/*    case 'delete-user':
        deleteUser(tab);
        break;
    ...
    case 'create-access-key':
        return createGen('create-access-key', enteredArgs, nextStep);
