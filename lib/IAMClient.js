...
class VaultClient {
    ...
    getAccount(options, callback) {
        ...
        if (accountArn === undefined
            ...
            assert(false, 'account-name, account-id, email or canonical-id need to be'
                + ' specified');
        }
        assert((accountArn, typeof accountArn === 'string' || 'arn should be a string'));
        assert((accountName, typeof accountName === 'string' || 'name should be a string'));
        assert((accountId, typeof accountId === 'string' || 'id should be a string'));
