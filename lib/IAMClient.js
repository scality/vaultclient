...
class VaultClient {
    ...
    constructor(host, port, useHttps, key, cert, ca, ignoreCa,
        accessKey, secretKeyValue, logApi, path, sessionToken, parameterValidation, supportsPostAuthV4) {
        ...
        this.supportsPostAuthV4 = supportsPostAuthV4 === true;
    }

    setSupportsPostAuthV4(supports = true) {
        this.supportsPostAuthV4 = supports;
    }
    ...
    verifySignatureV4(stringToSign, signature, accessKey, region, scopeDate,
        ...
        this.request(this.supportsPostAuthV4 ? 'POST' : 'GET', '/', false, (err, data, code) => {
            if (err) {
                return callback(err);
            }
            return callback(null, {
        ...
        }, data, options.reqUid, this.supportsPostAuthV4 ? 'application/json' : null);
    }
