...
class VaultClient {
    ...
    verifySignatureV4(stringToSign, signature, accessKey, region, scopeDate,
        ...
        console.log('VAULT_V4_AUTH_START', { requestId });
        ...
        console.log('VAULT_V4_AUTH_DATA_PREPARED', {
            requestId,
            action: data.Action,
            ...
            httpVerb,
            hasRequestContext: !!data.requestContext,
            hasSecurityToken: !!data.securityToken
        });
        ...
        this.request(httpVerb, '/', false, (err, data, code) => {
            ...
            if (err) {
                console.log('VAULT_V4_AUTH_FAILED', { 
                    requestId, 
                    error: err,
                    totalRequestTime,
                    requestStartTime: new Date(requestStartTime).toISOString(),
                    requestEndTime: new Date(requestEndTime).toISOString()
                });
                console.timeEnd(authTotalLabel);
                return callback(err);
            }
            console.log('VAULT_V4_AUTH_SUCCESS', {
                requestId,
                statusCode: code,
                responseData: data,
                totalRequestTime,
            ...
            // Performance summary
            console.log('VAULT_V4_AUTH_PERFORMANCE', {
                requestId,
                totalRequestTime,
                breakdown: {
                    requestTime: totalRequestTime,
                    // Note: Individual timing breakdowns are logged separately
                    // This helps identify where the "lost" time is spent
                },
                recommendations: totalRequestTime > 20 ? [
                    'High request time detected',
                    'Check network latency between client and vault',
                    'Monitor vault server performance',
                    'Consider connection pooling optimizations'
                ] : ['Request time within normal range']
            });
    ...
    async _signRequest(iamAuthenticate, req, options, path, timingLabels) {
        if (iamAuthenticate) {
            ...
            console.log('VAULT_V4_SIGNING_START');
            ...
            console.log('VAULT_V4_SIGNING_CREDENTIALS', {
                accessKeyId: this.accessKey ? `${this.accessKey.substring(0, 8)}...` : 'undefined',
                hasSecretKey: !!this.secretKeyValue,
            ...
            console.log('VAULT_V4_SIGNING_PROCESS', {
            ...
            console.log('VAULT_V4_SIGNING_COMPLETE', {
                signedHeaders: Object.keys(signedReq.headers),
                hasAuthorization: !!signedReq.headers.authorization,
                hasXAmzDate: !!signedReq.headers['x-amz-date'],
                hasXAmzSecurityToken: !!signedReq.headers['x-amz-security-token']
            });
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        if (isV4AuthRequest && timingLabels) {
            ...
            console.log('VAULT_V4_HTTP_REQUEST_START', {
                method,
        ...
        } else if (isV4AuthRequest && !timingLabels) {
            // Fallback for V4 auth requests without timing labels
            console.log('VAULT_V4_HTTP_REQUEST_START', {
                method,
                path: this._path || path,
                host: this.serverHost,
        ...
        if (isV4AuthRequest && timingLabels) {
            console.log('VAULT_V4_REQUEST_BODY_PREPARED', {
                bodyPreparationTime: bodyPreparationEnd - bodyPreparationStart,
                method,
                hasBody: !!options.body,
        ...
        if (isV4AuthRequest && timingLabels) {
            console.log('VAULT_V4_REQUEST_CREATED', {
                requestCreationTime: requestCreationEnd - requestCreationStart,
        ...
        req.on('response', res => {
            ...
            if (isV4AuthRequest && timingLabels) {
                console.log('VAULT_V4_HTTP_RESPONSE_RECEIVED', {
                    statusCode: res.statusCode,
                    statusMessage: res.statusMessage,
                    headers: res.headers,
                    responseStartTime: new Date(responseStartTime).toISOString()
                });
            }
            ...
            res.on('data', receivedData => {
                ...
                if (isV4AuthRequest && timingLabels) {
                    console.log('VAULT_V4_HTTP_CHUNK_RECEIVED', {
                        chunkIndex: chunks.length - 1,
                        chunkSize: receivedData.length,
                        totalChunksSoFar: chunks.length,
                        totalLengthSoFar: totalLength,
                        chunkTime: new Date(chunkTime).toISOString(),
                        timeSinceResponse: chunkTime - responseStartTime,
                        timeSinceFirstChunk: firstChunkTime ? chunkTime - firstChunkTime : 0
                    });
                }
                ...
                .on('error', err => {
                    if (isV4AuthRequest && timingLabels) {
                        console.log('VAULT_V4_HTTP_ERROR', { 
                            error: err.message,
                            chunksReceived: chunks.length,
                            totalLengthReceived: totalLength,
                            timeElapsed: Date.now() - responseStartTime
                        });
                ...
                .on('end', () => {
                    ...
                    if (isV4AuthRequest && timingLabels) {
                        console.log('VAULT_V4_HTTP_RESPONSE_COMPLETE', {
                            bodyLength: ret.length,
                            bodyPreview: ret.substring(0, 200) + (ret.length > 200 ? '...' : ''),
                            totalChunks: chunks.length,
                            totalLength: totalLength,
                            chunkSizes: chunks.map(chunk => chunk.length),
                            totalResponseTime,
                            dataProcessingTime,
                            timeToFirstChunk: firstChunkTime ? firstChunkTime - responseStartTime : 0,
                            timeToLastChunk: lastChunkTime ? lastChunkTime - responseStartTime : 0,
                            chunkTimings: chunkTimings.map(ct => ({
                                chunkIndex: ct.chunkIndex,
                                chunkSize: ct.chunkSize,
                                timeSinceResponse: ct.timeSinceResponse
                            })),
        ...
        req.on('error', err => {
            if (isV4AuthRequest && timingLabels) {
                console.log('VAULT_V4_HTTP_REQUEST_ERROR', { error: err.message });
                console.timeEnd(timingLabels.httpRequest);
            }
        ...
        this._signRequest(iamAuthenticate, req, options, path, timingLabels)
            ...
            .then(() => {
                ...
                const requestSendingEnd = Date.now();
                        ...
                        if (isV4AuthRequest && timingLabels) {
            console.log('VAULT_V4_REQUEST_SENT', {
                requestSendingTime: requestSendingEnd - requestSendingStart,
                method,
                hasBody: !!options.body,
                bodyLength: options.body ? options.body.length : 0
            });
    ...
    getObj(ret, cb) {
        ...
        if (ret.length === 0) {
            ...
            if (ret && ret.includes && ret.includes('AuthV4')) {
                console.log('VAULT_V4_PARSING_EMPTY', {
        ...
        if (ret[0] !== '{' && ret[0] !== '[') {
            return parseString(ret, {
                ...
                const parsingEndTime = Date.now();
                if (ret && ret.includes && ret.includes('AuthV4')) {
                    console.log('VAULT_V4_PARSING_XML', {
                        parsingTime: parsingEndTime - parsingStartTime,
                        responseLength: ret.length,
                        hasError: !!err
                    });
        ...
        const parsingEndTime = Date.now();
        if (ret && ret.includes && ret.includes('AuthV4')) {
            console.log('VAULT_V4_PARSING_JSON', {
                parsingTime: parsingEndTime - parsingStartTime,
                responseLength: ret.length,
    ...
    handleResponse(res, ret, log, cb, timingLabels) {
        ...
        this.getObj(ret, (err, obj) => {
            ...
            if (err) {
                            if (isV4AuthRequest) {
                console.log('VAULT_V4_RESPONSE_PARSE_ERROR', { 
                    error: err.message,
            ...
            if (res.statusCode < 400) {
                if (isV4AuthRequest) {
                    console.log('VAULT_V4_RESPONSE_PARSED', {
                        statusCode: res.statusCode,
                        responseObject: obj,
                        responseProcessingTime,
                        responseLength: ret.length,
                        processingStartTime: new Date(responseProcessingStart).toISOString(),
                        processingEndTime: new Date(responseProcessingEnd).toISOString()
                    });
                }
            ...
            if (obj && obj.ErrorResponse && obj.ErrorResponse.Error) {
                            if (isV4AuthRequest) {
                console.log('VAULT_V4_RESPONSE_VAULT_ERROR', {
                    errorCode: obj.ErrorResponse.Error.Code,
                    errorMessage: obj.ErrorResponse.Error.Message
                });
            }
            ...
            if (obj && obj.InternalError) {
                            if (isV4AuthRequest) {
                console.log('VAULT_V4_RESPONSE_INTERNAL_ERROR');
            }
                return cb(InternalError, null, res.statusCode);
            }
            if (isV4AuthRequest) {
                console.log('VAULT_V4_RESPONSE_UNKNOWN_ERROR', { error: obj });
            }
