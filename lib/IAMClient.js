...
class VaultClient {
     ...
     * @returns {undefined}
     */
    updateAccountQuota(quota, options, callback) {
        let accountName;
        let ci;
        if (options !== undefined && Object.keys(options).length !== 0) {
            ({ accountName, ci } = options);
        }
        const data = {
        ...
        if (ci) {
            ...
            if (accountName) {
                assert(typeof accountName === 'string' && accountName !== '', 'name should be a string');
                data.AccountName = accountName;
            }
        }
     ...
     * @returns {undefined}
     */
    deleteAccountQuota(options, callback) {
        const data = {
            Action: 'DeleteAccountQuota',
            Version: '2010-05-08',
        };
        if (options !== undefined && Object.keys(options).length !== 0) {
            const { accountName, ci } = options;
            if (accountName) {
                if (!ci) {
                    assert(typeof accountName === 'string' && accountName !== '',
     ...
     * @returns {undefined}
     */
    getAccountQuota(options, callback) {
        const data = {
