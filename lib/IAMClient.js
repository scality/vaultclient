...
class IAMClient {
                ...
                streams: [
                    { stream: process.stdout},
                    {
                        ...
                        stream: bunyanLogstash.createStream({
                            host: config.log.logstash.host,
                            port: config.log.logstash.port,
                        }),
                    }
                ],
            }
    ...
    get(command, data, metadata, callback) {
        ...
        const log = this.logger.newRequestLogger(finalData[requestUidKey]);
        log.trace('sending GET request to vault', { data, });
        this.request('GET', path, callback, finalData, log);
    }
     ...
     *  @returns {void}
     */
    request(method, path, callback, data, log) {
        const options = {
            method,
            path,
        ...
        req.on('response', (res) => {
            ...
            .on('end', () => {
                log.trace('received response from vault');
