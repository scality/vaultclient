...
class VaultClient {
    ...
    /**
     * Create a super admin login profile
     *
     ...
     * @returns {undefined}
     */
    createSuperAdminLoginProfile(username, password, callback) {
        assert(typeof username === 'string' && username !== '',
            'username is required');
        assert(typeof password === 'string' && password !== '',
            'password is required');
        const data = {
            Action: 'CreateSuperAdminLoginProfile',
            Version: '2010-05-08',
            UserName: username,
            Password: password,
        };
    ...
    /**
     * Get a super admin login profile
     *
     * @param {string} username - super admin username
     * @param {VaultClient~requestCallback} callback - callback
     * @returns {undefined}
     */
    getSuperAdminLoginProfile(username, callback) {
        assert(typeof username === 'string' && username !== '',
            'username is required');
        const data = {
            Action: 'GetSuperAdminLoginProfile',
            Version: '2010-05-08',
            UserName: username,
        };
        this.request('POST', '/', true, (err, result) => {
     ...
     * @returns {undefined}
     */
    updateSuperAdminPassword(username, newPassword, callback) {
        assert(typeof username === 'string' && username !== '',
            'username is required');
        assert(typeof newPassword === 'string' && newPassword !== '',
            'newPassword is required');
        const data = {
            Action: 'UpdateSuperAdminLoginProfile',
            Version: '2010-05-08',
            UserName: username,
    ...
    /**
     * Delete super admin login profile
     *
     * @param {string} username - super admin username
     * @param {VaultClient~requestCallback} callback - callback
     * @returns {undefined}
     */
    deleteSuperAdminLoginProfile(username, callback) {
        assert(typeof username === 'string' && username !== '',
            'username is required');
        const data = {
            Action: 'DeleteSuperAdminLoginProfile',
            Version: '2010-05-08',
            UserName: username,
        };
        this.request('POST', '/', true, (err, result) => {
    ...
    /**
     * List all super admin login profiles
     *
     * @param {VaultClient~requestCallback} callback - callback
     * @returns {undefined}
     */
    listSuperAdminLoginProfiles(callback) {
        const data = {
            Action: 'ListSuperAdminLoginProfiles',
            Version: '2010-05-08',
        };
        this.request('POST', '/', true, (err, result) => {
            if (err) {
        ...
        this.getObj(ret, (err, obj) => {
            ...
            if (obj && obj.ErrorResponse && obj.ErrorResponse.Error) {
                ...
                return cb({
                    ...
                    [obj.ErrorResponse.Error.Code]: true,
                },
                null,
                res.statusCode
                );
