...
class IAMClient {
    ...
    /**
     * A simple wrapper for account creation
     * @param{Object} dict - dictionary containing all required arguments:
     * @param{String} dict.name - account name
     * @param{String} dict.emailAddress - account email address
     * @param{String} dict.saltedPwd - account password
     * @param{Function} callback - callback contains a json with account
     *  identifiers returned by the server
     * @returns{Void}
     */
    // TODO: change 'saltedPwd' to 'password', also in the server
    createAccount(dict, callback) {
        this.create('create-account', dict, '', callback);
    }

    /**
     * A simple wrapper for user creation
     * @param{Object} dict - dictionary containin all required arguments:
     * @param{String} dict.accountName - parent account name
     * @param{String} dict.name - user name
     * @param{String} dict.emailAddress - user email address
     * @param{String} dict.saltedPasswd - user password (not salted)
     * @param{Function} callback - callback contains a json with user
     *  identifiers returned by the server
     * @returns{Void}
     */
    // TODO: change 'saltedPasswd' to 'password', also in the server
    createUser(dict, callback) {
        this.create('create-user', dict, '', callback);
    }

    /**
     * A simple wrapper for access key creation
     * @param{Object} dict - dictionary containing all required arguments:
     * @param{String} dict.accountName - account name
     * @param{String} dict.userName - user Name (if the key is created for an
     *  account then set userName to 0)
     * @param{Function} callback - callback contains a json with access key
     *  data such as identifier and value
     * @returns{Void}
     */
    createAccessKey(dict, callback){
        this.create('create-access-key', dict, '', callback);
    }

    /**
     * A simple wrapper for account deletion
     * @param{Object} dict - dictionary  containing all required arguments:
     * @param{String} dict.name - account name
     * @param{Function} callback - callback contains the HTTP result of the
     *  operation
     * @returns{Void}
     */
    deleteAccount(dict, callback) {
        this.delete('delete-account', '', dict, callback);
    }

    /**
     * A simple wrapper for user deletion
     * @param{Object} dict - dictionary containing all required arguments:
     * @param{String} dict.accountName - parent account name
     * @param{String} dict.userName - user name
     * @param{Function} callback - callback contains the HTTP result of the
     *  operation
     * @returns{Void}
     */
    deleteUser(dict, callback) {
        this.delete('delete-user', '', dict, callback);
    }

    /**
     * A simple wrapper for access key deletion
     * @param{Object} dict - dictionary containing all required arguments:
     * @param{String} dict.secretKeyId - access key id (secret key id in Vault's
     *  jargon)
     * @param{Function} callback - callback contains the HTTP result of the
     *  operation
     * @returns{Void}
     */
    deleteAccessKey(dict, callback) {
        this.delete('delete-access-key', '', dict, callback);
    }
