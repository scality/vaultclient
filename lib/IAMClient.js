...
class VaultClient {
    ...
    createAccount(accountName, options, callback) {
        ...
        const { quota, externalAccountId, cdTenantIdList } = options;
        assert((cdTenantIdList && Array.isArray(cdTenantIdList))
            || !cdTenantIdList,
        'cloud director tenant id list should be an array');
        if (quota) {
        ...
        if (cdTenantIdList) {
            data.cdTenantIdList = cdTenantIdList;
        }
