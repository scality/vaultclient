...
class VaultClient {
    ...
    _trackSocketUsage(socket, reqId) {
        ...
        // Use socket object identity + connection info for unique tracking
        const socketId = socket._handle ? socket._handle.fd : Math.random().toString(36);
        const remoteAddr = socket.remoteAddress || 'connecting';
        const remotePort = socket.remotePort || 'connecting';
        const socketKey = `${socketId}-${remoteAddr}:${remotePort}`;
    ...
    _logSocketStats() {
        ...
        this._agentPool.forEach((agent, index) => {
            ...
            if (stats.createSocketCount) agentStats.totalSockets += stats.createSocketCount;
            if (stats.requestCount) agentStats.totalRequests += stats.requestCount;
        ...
        console.log('=== VAULT SOCKET STATISTICS ===', {
            ...
            }).sort((a, b) => b.requests - a.requests).slice(0, 10), // Top 10 most used sockets
