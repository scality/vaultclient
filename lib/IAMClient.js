...
class IAMClient {
    ...
    createAccount(accountName, options, callback) {
        ...
        this.request('POST', '/', (err, result) => {
            ...
            return callback(null, {
                ...
                secretKey: result.secretKey.data,
            });
        }, {
    ...
    request(method, path, callback, data) {
        ...
        if (method === 'POST' && typeof data === 'object') {
            ...
            Object.keys(data).forEach(key => {
                ...
                paramsToSend.push(`${formattedKey}=${formattedValue}`);
            });
            req.write(paramsToSend.join(''));
    ...
    getObj(ret, cb) {
        ...
        try {
            ...
            err = new Error(`cannot decode vault response: ${error}`);
            err.raw = ret;
        }
        if (err) {
            return cb(err);
    ...
    handleResponse(res, ret, callback) {
        this.getObj(ret, (err, obj) => {
            ...
            return callback(errors[obj.Error.Code], null, res.statusCode);
        });
    }
}
