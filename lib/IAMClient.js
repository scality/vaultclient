...
class VaultClient {
    ...
    constructor(host, port, useHttps, key, cert, ca, ignoreCa,
        ...
        // Socket monitoring with concurrent tracking
        this._socketStats = {
            totalRequests: 0,
    ...
    _trackSocketUsage(socket, reqId) {
        ...
        // Use socket object as unique identifier - no more string keys
        if (!socket._vaultSocketKey) {
            socket._vaultSocketKey = `socket-${Math.random().toString(36).substr(2, 9)}`;
        }
        
        const socketKey = socket._vaultSocketKey;
        const socketId = socket._handle ? socket._handle.fd : 'pending';
        const remoteAddr = socket.remoteAddress || 'pending';
        const remotePort = socket.remotePort || 'pending';
        ...
        if (!this._socketStats.socketMetrics.has(socketKey)) {
            this._socketStats.socketMetrics.set(socketKey, {
                ...
                created: now,
                lastUsed: now,
                socketId: `${socketId}-${remoteAddr}:${remotePort}`,
            ...
            // Log new socket creation every 100 new sockets
            if (this._socketStats.socketMetrics.size % 100 === 0) {
                console.log(`[SOCKET_TRACKING] Now tracking ${this._socketStats.socketMetrics.size} sockets (${socketKey} -> ${socketId}-${remoteAddr}:${remotePort})`);
            }
        } else {
            // Update display info as socket connects
            const metric = this._socketStats.socketMetrics.get(socketKey);
            metric.socketId = `${socketId}-${remoteAddr}:${remotePort}`;
            metric.fd = socketId;
        }
    ...
    _logSocketStats() {
        ...
        // Check actual agent socket usage
        ...
        // Compare agent stats vs our tracking
        console.log(`[SOCKET_DISCREPANCY] Agent Stats: ${agentStats.totalSockets} sockets, ${agentStats.totalRequests} requests | Our Tracking: ${totalSockets} sockets, ${totalRequests} requests`);
        ...
        console.log('=== VAULT SOCKET STATISTICS ===', {
            ...
            requestsPerSecond: Math.round(totalRequests / ((now - this._socketStats.lastStatsLog) / 1000))
        });
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        req.on('socket', socket => {
            ...
            this._logSocketStats(); // Periodic stats logging
            
            // Log socket creation and HTTP version
            if (this._socketStats.totalRequests % 500 === 0) {
                console.log(`[HTTP_VERSION] Using HTTP/${socket.parser?.incoming?.httpVersion || '1.1'} to ${this.serverHost}:${this.serverPort}`);
                console.log(`[SOCKET_USAGE] Agent ${socket._vaultAgentIndex}: FD ${socket._handle?.fd}, Created: ${socket.connecting ? 'YES' : 'NO'}`);
            }
        });

        req.on('response', res => {
            // Log HTTP version from response (more reliable)
            if (this._socketStats.totalRequests % 1000 === 0) {
                console.log(`[HTTP_VERSION] Response HTTP/${res.httpVersion || '1.1'} from ${this.serverHost}:${this.serverPort}`);
            }
