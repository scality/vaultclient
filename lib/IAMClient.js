...
class VaultClient {
    ...
    verifySignatureV4(stringToSign, signature, accessKey, region, scopeDate,
        options, callback) {
        console.time('V4 Auth Total');
        console.log('V4 Auth: Starting authentication process');
        ...
        console.log('V4 Auth: Request data prepared', {
            action: data.Action,
        ...
        this.request(httpVerb, '/', false, (err, data, code) => {
            if (err) {
                console.log('V4 Auth: Authentication failed', { error: err });
                console.timeEnd('V4 Auth Total');
                return callback(err);
            }
            console.log('V4 Auth: Authentication successful', {
                statusCode: code,
                responseData: data
            });
            console.timeEnd('V4 Auth Total');
    ...
    async _signRequest(iamAuthenticate, req, options, path) {
        if (iamAuthenticate) {
            console.time('V4 Signing');
            console.log('V4 Signing: Starting request signing process');
            ...
            console.log('V4 Signing: Creating signer with credentials', {
            ...
            console.time('V4 Sign Process');
            console.log('V4 Signing: Signing request with options', {
            ...
            console.log('V4 Signing: Request signed successfully', {
            ...
            console.timeEnd('V4 Sign Process');
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        if (isV4AuthRequest) {
            console.time('V4 HTTP Request');
            console.log('V4 HTTP: Starting HTTP request', {
                method,
        ...
        req.on('response', res => {
            if (isV4AuthRequest) {
                console.log('V4 HTTP: Received response', {
            ...
            res.on('data', receivedData => {
                ...
                .on('error', err => {
                    if (isV4AuthRequest) {
                        console.log('V4 HTTP: Error receiving data', { error: err.message });
                        console.timeEnd('V4 HTTP Request');
                    }
                ...
                .on('end', () => {
                    ...
                    if (isV4AuthRequest) {
                        console.log('V4 HTTP: Response body received', {
        ...
        req.on('error', err => {
            if (isV4AuthRequest) {
                console.log('V4 HTTP: Request error', { error: err.message });
                console.timeEnd('V4 HTTP Request');
            }
    ...
    handleResponse(res, ret, log, cb) {
        ...
        this.getObj(ret, (err, obj) => {
            if (err) {
                if (isV4AuthRequest) {
                    console.log('V4 Response: Error parsing response', { error: err.message });
                }
            ...
            if (res.statusCode < 400) {
                if (isV4AuthRequest) {
                    console.log('V4 Response: Successfully parsed response', {
                        statusCode: res.statusCode,
                        responseObject: obj
                    });
                }
            ...
            if (obj && obj.ErrorResponse && obj.ErrorResponse.Error) {
                if (isV4AuthRequest) {
                    console.log('V4 Response: Vault error response', {
            ...
            if (obj && obj.InternalError) {
                if (isV4AuthRequest) {
                    console.log('V4 Response: Internal error from vault');
                }
                return cb(InternalError, null, res.statusCode);
            }
            if (isV4AuthRequest) {
                console.log('V4 Response: Unable to translate error from vault', { error: obj });
            }
