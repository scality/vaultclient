...
const regexAccessKey = /^[A-Z0-9]{20}$/;
...
class VaultClient {
   ...
   */
    constructor(
        ...
        assert(
            cert === undefined || typeof cert === 'string',
            'cert must be a string'
        );
        assert(ca === undefined
            || typeof ca === 'string', 'ca must be a string');
   ...
   * @param {string} options.externalAccountId - user-supplied account ID
   ...
   */
    createAccount(accountName, options, callback) {
        ...
        if (options.quota !== undefined && options.quota !== null) {
            ...
            assert(
                typeof options.quota === 'number' && !conv && options.quota > 0,
        ...
        if (
            ...
            const conv = Number.isNaN(
                Number.parseInt(options.externalAccountId, 10)
            );
            assert(typeof options.externalAccountId === 'string'
                && options.externalAccountId !== ''
                && regexAccountId.test(options.externalAccountId)
                && !conv, 'Invalid Account ID supplied');
   ...
   *
   * @param {string} accountName - account name
   * @param {object} options - user injected credentials
   * @param {VaultClient~requestCallback} callback - callback
   * @returns {undefined}
   */
    generateAccountAccessKey(accountName, options, callback) {
        assert(
            typeof accountName === 'string' && accountName !== '',
            'accountName is required'
        );
        ...
        if (
            options.externalAccessKey !== undefined
            && options.externalAccessKey !== null
        ) {
            assert(
                typeof options.externalAccessKey === 'string'
                && options.externalAccessKey !== ''
                && regexAccessKey.test(options.externalAccessKey),
                'Invalid Access Key supplied'
            );
            data.externalAccessKey = options.externalAccessKey;
        }
        if (
            options.externalSecretKey !== undefined
            && options.externalSecretKey !== null
        ) {
            assert(
                typeof options.externalSecretKey === 'string'
   ...
   */
    updateAccountQuota(accountName, quota, callback) {
        ...
        assert(
            typeof quota === 'number' && !conv && quota > 0,
            'Quota must be a positive number'
        );
        this.request('POST', '/', true, callback, {
   ...
   */
    getAccounts(accountIds, emailAddresses, canonicalIds, options, callback) {
        ...
        assert((canonicalIds && Array.isArray(canonicalIds))
            || !canonicalIds, 'canonicalIds should be an array');
        if (
            ...
            assert(false,
                'accountIds, emailAddresses and canonicalIds '
                + 'ids are exclusive');
        }
        const data = {
            Action: 'GetAccounts',
   ...
   */
    listAccounts(options, callback) {
        ...
        if (typeof options.maxItems !== 'undefined') {
            assert.strictEqual(
                typeof options.maxItems,
                'number',
                'maxItems need to be a number'
            );
            const conv = Number.isNaN(Number.parseInt(options.maxItems, 10));
   ...
   */
    verifySignatureV4(
        ...
        assert(
            typeof stringToSign === 'string' && stringToSign !== '',
        ...
        assert(
            typeof scopeDate === 'string' && scopeDate !== '',
   ...
   */
    getCanonicalIdsByAccountIds(accountIds, options, callback) {
        ...
        accountIds.every(
            item => assert(typeof item === 'string'),
   ...
   */
    checkPolicies(requestContextParams, userArn, options, callback) {
        assert(
            typeof requestContextParams === 'object',
            'need requestContextParams'
        );
        assert(
            typeof requestContextParams.constantParams === 'object',
            'need constantParams'
        );
    ...
    healthcheck(reqUid, callback) {
        this.request('GET', '/_/healthcheck', false,
            callback, null, reqUid, null);
    }
   ...
   * @returns {undefined}
   */
    request(method, path, iamAuthenticate, callback,
        data, reqUid, contentType) {
        const log = reqUid
        ...
        const req = this.useHttps ? https.request(options)
    ...
    handleResponse(res, ret, log, cb) {
        this.getObj(ret, (err, obj) => {
            ...
            if (obj && obj.ErrorResponse && obj.ErrorResponse.Error) {
                ...
                return cb(errors[obj.ErrorResponse.Error.Code],
                    null, res.statusCode);
            }
            ...
            return cb(
                errors.InternalError.customizeDescription(
                    'unable to translate error from vault'
                )
            );
        });
    }
