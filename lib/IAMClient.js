...
const disableLogs = process.env.DISABLE_LOG_VLTCLT;
...
class VaultClient {
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        // console.time(rd1+'REQUEST')
        // console.time(rd2+'TIMETODATA')
        let cnt  = 0;
        // request events
        req.on('response', res => {
            // response events
            res.on('data', receivedData => {
                // console.timeEnd(rd2+'TIMETODATA')
                ...
                .on('end', () => {
                    // console.log(rd1+'DATATOTAL:', cnt)
                    // console.timeEnd(rd1+'REQUEST')
                    //this.handleResponse(res, Buffer.concat(ret).toString(), log, callback);
                    return this.handleResponse(res, ret, log, callback);
                });
        });
        req.on('error', err => {
