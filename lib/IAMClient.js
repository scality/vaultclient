...
class VaultClient {
    ...
    constructor(host, port, useHttps, key, cert, ca, ignoreCa,
        accessKey, secretKeyValue, securityToken, logApi, path, sessionToken) {
     ...
     * @returns {undefined}
     */
    createAccount(accountName, options, callback, webIdentityToken) {
        ...
        this.request('POST', '/', webIdentityToken ? false : true, (err, result) => {
            if (err) {
                return callback(err);
            }
        ...
        }, data, undefined, undefined, webIdentityToken);
    }
     ...
     * @returns {undefined}
     */
    generateAccountAccessKey(accountName, callback, options, webIdentityToken) {
        assert(typeof accountName === 'string' && accountName !== '',
            'accountName is required');
        const data = {
     ...
     * @returns {undefined}
     */
    deleteAccount(accountName, callback, webIdentityToken) {
        assert(typeof accountName === 'string' && accountName !== '',
            'accountName is required');

        this.request('POST', '/', webIdentityToken ? false : true, callback, {
     ...
     * @returns {undefined}
     */
    updateAccountQuota(accountName, quota, callback, webIdentityToken) {
        const conv = Number.isNaN(Number.parseInt(quota, 10));
        assert(typeof accountName === 'string' && accountName !== '',
            'accountName is required');
     ...
     * @returns {undefined}
     */
    deleteAccountQuota(accountName, callback, webIdentityToken) {
        assert(typeof accountName === 'string' && accountName !== '',
            'accountName is required');
        this.request('POST', '/', webIdentityToken ? false : true, callback, {
     ...
     * @return {undefined}
     */
    listAccounts(options, callback, webIdentityToken) {
        const data = {
            Action: 'ListAccounts',
            Version: '2010-05-08',
        };
        ...
        this.request('POST', '/', webIdentityToken ? false : true, callback, data, undefined, undefined, webIdentityToken);
    }
     ...
     * @return {undefined}
     */
    getAccount(options, callback, webIdentityToken) {
        const {
            accountArn,
            accountName,
            accountId,
    ...
    checkPermissions(actionParams, options, callback, webIdentityToken) {
        assert(typeof actionParams === 'string',
            'need actionParams');
        if (webIdentityToken) {
            assert(typeof webIdentityToken === 'string', 'WebIdentity token must be a string');
        }
        const data = {
            Action: 'CheckPermissions',
            actionParams,
        };
        this.request('POST', '/', webIdentityToken ? false : true, (err, data, code) => {
            if (err) {
                return callback(err);
            }
            return callback(null, {
        ...
        }, data, options.reqUid, 'application/json', webIdentityToken);
    }
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        if (webIdentityToken) {
            req.setHeader('authorization', `Bearer ${webIdentityToken}`);
        }
        if (method === 'POST') {
