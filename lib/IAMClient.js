...
class VaultClient {
    ...
    constructor(host, port, useHttps, key, cert, ca, ignoreCa,
        ...
        const maxSockets = process.env.USE_INFINITY_SOCKETS ? Infinity : parseInt(process.env.VAULT_MAX_SOCKETS, 10) || 100;
    ...
    _logRequestTiming(timing) {
        // Only log every 100th request to avoid spam, or if there's an error
        const shouldLog = timing.error || (this._socketStats.totalRequests % 100 === 0);
        
        if (shouldLog) {
            const breakdown = {
                requestId: timing.requestId,
                total: Number((timing.callbackEnd - timing.start).toFixed(3)) + 'ms',
                socketAssignment: Number((timing.socketAssigned - timing.start).toFixed(3)) + 'ms',
                connection: Number((timing.connectionEnd - timing.connectionStart).toFixed(3)) + 'ms',
                requestSend: Number((timing.requestSent - timing.connectionEnd).toFixed(3)) + 'ms',
                networkWait: Number((timing.firstByte - timing.requestSent).toFixed(3)) + 'ms',  
                responseReceive: Number((timing.responseEnd - timing.firstByte).toFixed(3)) + 'ms',
                callbackProcess: Number((timing.callbackEnd - timing.callbackStart).toFixed(3)) + 'ms',
                error: timing.error
            };
            
            console.log('=== REQUEST TIMING BREAKDOWN ===', breakdown);
            
            const networkWaitMs = timing.firstByte - timing.requestSent;
            const connectionMs = timing.connectionEnd - timing.connectionStart;
            
            // If network wait is high, this indicates where the real bottleneck is
            if (networkWaitMs > 20) {
                console.log(`[HIGH_LATENCY] Network wait ${networkWaitMs.toFixed(3)}ms suggests network/server bottleneck`);
            }
            if (connectionMs > 10) {
                console.log(`[SLOW_CONNECTION] Connection setup ${connectionMs.toFixed(3)}ms suggests socket reuse issue`);
            }
            if (networkWaitMs < 1) {
                console.log(`[VERY_FAST] Network wait only ${networkWaitMs.toFixed(3)}ms - bottleneck likely elsewhere`);
            }
        }
    }
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        // DETAILED REQUEST TIMING (microsecond precision)
        const requestTiming = {
            requestId,
        ...
        req.on('socket', socket => {
            requestTiming.socketAssigned = performance.now();
            
            // Add agent info to socket tracking
            socket._vaultAgentIndex = agentIndex; // Store the actual agent index used
            
            // Track connection timing
            if (socket.connecting) {
                requestTiming.connectionStart = performance.now();
                socket.once('connect', () => {
                    requestTiming.connectionEnd = performance.now();
                    this._trackSocketUsage(socket, requestId);
                    this._logSocketStats();
                });
            } else {
                // Already connected (reused socket)
                requestTiming.connectionStart = requestTiming.socketAssigned;
                requestTiming.connectionEnd = requestTiming.socketAssigned;
                this._trackSocketUsage(socket, requestId);
                this._logSocketStats();
            }
        ...
        req.on('response', res => {
            requestTiming.firstByte = performance.now();
            ...
            res.on('data', receivedData => {
                ...
                .on('error', err => {
                    requestTiming.error = err.message;
                    requestTiming.responseEnd = performance.now();
                    this._completeSocketUsage(requestId);
                    this._logRequestTiming(requestTiming);
                ...
                .on('end', () => {
                    ...
                    requestTiming.callbackStart = performance.now();
                    this.handleResponse(res, ret, log, (err, result, statusCode, statusMessage) => {
                        requestTiming.callbackEnd = performance.now();
                        this._logRequestTiming(requestTiming);
                        callback(err, result, statusCode, statusMessage);
                    }, isAuthv4);
                });
        });
        req.on('error', err => {
        ...
        this._signRequest(iamAuthenticate, req, options, path)
            ...
            .then(() => {
                ...
                requestTiming.requestSent = performance.now();
                req.end();
