...
class VaultClient {
     ...
     * @returns {undefined}
     */
    createAccount(accountName, options, webIdentityToken, callback) {
     ...
     * @return {undefined}
     */
    listAccounts(options, webIdentityToken, callback) {
        const data = {
            Action: 'ListAccounts',
            Version: '2010-05-08',
        };
    ...
    checkPolicies(requestContextParams, userArn, options, callback) {
        ...
        console.log('VAULTCLIENT SENDING', data);
        this.request('POST', '/', false, (err, data, code) => {
            console.log('VAULTCLIENT GETTING', err, data, code);
            if (err) {
                return callback(err);
            }
            return callback(null, {
    ...
    checkPermissions(actionParams, webIdentityToken, options, callback) {
