...
class VaultClient {
    ...
    getAccount(options, callback) {
        ...
        if (arn === undefined
            ...
            assert(false, 'name, id, emailAddress or canonicalId need to be specified');
        }
        assert((arn, typeof arn === 'string' || 'arn should be a string'));
        assert((name, typeof name === 'string' || 'name should be a string'));
        assert((id, typeof id === 'string' || 'id should be a string'));
        assert((canonicalId, typeof canonicalId === 'string' || 'canonicalId should be a string'));
        assert((emailAddress, typeof emailAddress === 'string' || 'emailAddress should be a string'));
        const data = {
            Action: 'GetAccount',
            Version: '2010-05-08',
        };
        ...
        if (Object.values(data).length > 3) {
            assert(false, 'arn, name, id, emailAddress and canonicalId IDs are exclusive');
        }
        this.request('POST', '/', true, callback, data);
    }
