...
class IAMClient {
    ...
    verifySignatureV2(string, signature, accessKey, options, callback) {
        ...
        this.request('GET', '/', false, (err, data, code) => {
            if (err) {
                if (logger) {
                    logger.error('error verifying signature v2', {
                        error: err,
                        method: 'IAMClient.verifySignatureV2',
    ...
    verifySignatureV4(stringToSign, signature, accessKey, region, scopeDate,
        ...
        this.request('GET', '/', false, (err, data, code) => {
            if (err) {
                if (logger) {
                    logger.error('error verifying signature v4', {
                        error: err,
                        method: 'IAMClient.verifySignatureV4',
                        component: 'vaultclient',
                    });
                }
                return callback(err);
        ...
        }, data, options.reqUid, null, logger);
    }
    ...
    getEmailAddresses(canonicalIds, options, callback) {
        ...
        this.request('GET', '/', false, (err, data, code) => {
            if (err) {
                if (logger) {
                    logger.error('error getting email addresses', {
                        error: err,
                        method: 'IAMClient.getEmailAddresses',
                        component: 'vaultclient',
                    });
                }
                return callback(err);
    ...
    getCanonicalIds(emailAddresses, options, callback) {
        ...
        this.request('GET', '/', false, (err, data, code) => {
            if (err) {
                if (logger) {
                    logger.error('error getting canonical Ids', {
                        error: err,
                        method: 'IAMClient.getCanonicalIds',
                        component: 'vaultclient',
                    });
                }
                return callback(err);
    ...
    checkPolicies(requestContextParams, userArn, options, callback) {
        ...
        }, data, options.reqUid, 'application/json', options.logger);
    }
    ...
    request(method, path, iamAuthenticate, callback, data, reqUid,
        ...
        req.on('response', res => {
            ...
            .on('error', err => {
                if (logger) {
                    logger.error('error receiving data from vault', {
                        component: 'vaultclient',
                        method: 'IAMClient:request()',
                        error: err.stack || err,
                    });
                    return callback(errors.InternalError.customizeDescription(
                        'vaultclient: error receiving data from vault'));
                }
                return callback(err);
            })
        ...
        req.on('error', err => {
            if (logger) {
                logger.error('error sending a request to vault', {
                    component: 'vaultclient',
                    method: 'IAMClient:request()',
                    error: err.stack || err,
                });
                return callback(errors.InternalError.customizeDescription(
                    'vaultclient: error sending a request to vault'));
            }
            return callback(err);
        });
        req.end();
     ...
     * @param {object} logger - werelogs request logger
     * @param {IAMClient~requestCallback} callback - callback
     * @returns {undefined}
     */
    handleResponse(res, ret, logger, callback) {
        this.getObj(ret, (err, obj) => {
            ...
            if (logger) {
                logger.error('vault response contains error', {
                    error: retErr,
