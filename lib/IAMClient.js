...
class VaultClient {
        ...
        for (let i = 0; i < agentCount; i++) {
            ...
            if (this.useHttps) {
                ...
                agent = new HttpAgent.Agent({
                    keepAlive: true,
                    freeSocketTimeout: httpClientFreeSocketTimeout,
                    maxSockets: 3,                    // ← Force socket cycling
                    maxFreeSockets: 1,                // ← Prevent socket hoarding  
    ...
    _trackSocketUsage(socket, reqId) {
        ...
        // Use consistent socket identity - assign unique ID if needed
        if (!socket._vaultSocketId) {
            socket._vaultSocketId = socket._handle ? 
                socket._handle.fd : 
        ...
        const socketKey = `${socket._vaultSocketId}-${remoteAddr}:${remotePort}`;
        
        // Handle socket key migration from pending to connected state
        const pendingKey = `${socket._vaultSocketId}-pending:pending`;
        let existingMetric = null;
        
        if (socketKey !== pendingKey && this._socketStats.socketMetrics.has(pendingKey)) {
            // Socket connected - migrate from pending to real address
            existingMetric = this._socketStats.socketMetrics.get(pendingKey);
            this._socketStats.socketMetrics.delete(pendingKey);
            this._socketStats.concurrentBySocket.delete(pendingKey);
            this._socketStats.maxConcurrentPerSocket.delete(pendingKey);
        }
        
        if (!this._socketStats.socketMetrics.has(socketKey)) {
            this._socketStats.socketMetrics.set(socketKey, existingMetric || {
                requests: 0,
                totalLatency: 0,
                created: now,
                lastUsed: now,
    ...
    _logSocketStats() {
        ...
        console.log('=== VAULT SOCKET STATISTICS ===', {
            ...
            }).sort((a, b) => b.requests - a.requests).slice(0, 15), // Top 15 sockets for readability
